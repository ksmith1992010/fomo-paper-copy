...
function remainingHeld(row) {
  for (const field of ["remainingAmount", "currentAmount", "amountRemaining", "balance"]) {
    if (row?.[field] == null || row[field] === "") continue;
    const remaining = num(row[field]);
    return remaining;
  }
  return null;
}

/** Fraction of the leader's original position that is sold. Amounts win over a closed flag. */
function leaderSoldFraction(row, sold, bought) {
  const remaining = remainingHeld(row);
  if (remaining != null && remaining <= 0 && sold > 0) return 1;
  if (bought > 0 && sold > 0) {
    const fraction = sold / bought;
    if (fraction >= 0.999) return 1;
    return Math.min(1, fraction);
  }
  if (row?.closedAt) return 1;
  if (remaining != null && sold > 0) {
    const before = remaining + sold;
    if (!(before > 0) || remaining <= before * 0.001) return 1;
    return Math.min(1, sold / before);
  }
  return 1;
}

export function printsFromTrades(userId, payload) {
  const prints = [];
  for (const row of payload?.trades || []) {
    ...
    if (mint && sold > 0 && exit > 0) {
      const exitFraction = leaderSoldFraction(row, sold, bought);
      prints.push({
        id: `${row.tradeId}:sell:${exitFraction.toFixed(6)}`,
        ...
        traderId: userId,
        markUsd: num(row.priceUsd),
        positionId: row.tradeId || "",
        exitFraction,
        soldQty: sold,
        heldQty: bought > 0 ? bought : sold + (remainingHeld(row) || 0),
      });
    }
  }
  return prints;
...
export async function quoteMints(mints, options = {}) {
  const fetcher = options.fetchImpl || globalThis.fetch;
  const routes = options.routes || [];
  const wanted = [];
  for (const mint of mints || []) {
    if (mint && !wanted.some((item) => sameMint(item, mint))) wanted.push(mint);
  }
  const dexMarks = {};
  const liquidity = {};
  const symbols = {};
  const tokenImages = {};
  const dexResults = await Promise.all(wanted.map((mint) => getJson(`${DEX_TOKENS}/${encodeURIComponent(mint)}`, routes, {}, fetcher)));
  for (const dex of dexResults) {
    for (const pair of dex?.pairs || []) {
      ...
      const known = wanted.find((item) => sameMint(item, mint));
      if (!known) continue;
  ...
  return { dexMarks, symbols, tokenImages, routes };
}
...
async function attachMarks(traders, routes, fomoMarks, fetcher) {
  ...
  const quoted = await quoteMints(mints, { fetchImpl: fetcher, routes });
  const dexMarks = quoted.dexMarks;
  const symbols = quoted.symbols;
  const tokenImages = quoted.tokenImages;
  for (const trader of traders) {
