import { snapshotBanner, snapshotFromBoard } from "./board-snapshot.js";
...
async function followSnapshot(snapshot, routes, fetcher, status) {
  const solUsd = await solPriceUsd(routes, fetcher);
  const traders = [];
  for (const saved of snapshot.traders) {
    const prints = saved.wallet
      ? await recentSwaps(saved.wallet, { fetchImpl: fetcher, solUsd, limit: 8 })
      : [];
    const ready = prints.map((print) => ({ ...print, traderId: saved.userId, traderName: saved.handle }));
    traders.push({
      rank: saved.rank,
      id: saved.userId,
      userId: saved.userId,
