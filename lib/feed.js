import { sleevesFor } from "./paper.js";
...
export async function loadFeed(options = {}) {
  ...
  for (let offset = 0; offset < mints.length; offset += 30) {
    const chunk = mints.slice(offset, offset + 30);
    const dex = await getJson(`${DEX_TOKENS}/${chunk.join(",")}`, routes);
  ...
  const marks = { ...fomoMarks };
  for (const [mint, price] of Object.entries(dexMarks)) marks[mint] = price;
